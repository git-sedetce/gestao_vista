    ...
    try {
      const eventos = await database.Evento.findAll({
        ...
        attributes: [
          ...
          "tipo_evento_id",
          [
            Sequelize.fn("COUNT", Sequelize.col("tipo_evento_id")),
            "qtd_tipo_evento",
          ],
        ],
        include: [
    ...
    try {
      const eventos = await database.Evento.findAll({
        ...
        group: [
          ...
          "sexec_id",
          "ass_evento_sexec.id",
          "ass_evento_sexec.secretaria",
          "ass_evento_sexec.sigla",
        ],
      });
      return res.status(200).json(eventos);
    } catch (error) {
    ...
    try {
      const eventos = await database.Evento.findAll({
        ...
        attributes: [
          ...
          "participacao_id",
          [
            Sequelize.fn("COUNT", Sequelize.col("participacao_id")),
            "qtd_participacao",
          ],
        ],
        include: [
        ...
        group: [
          "ano",
          "participacao_id",
          "ass_evento_participacao.id",
          "ass_evento_participacao.participacao",
        ],
      });
      return res.status(200).json(eventos);
    } catch (error) {
    ...
    try {
      const eventos = await database.Evento.findAll({
        ...
        attributes: [
          ...
          "recursos_id",
          [
            Sequelize.fn("COUNT", Sequelize.col("Evento.recursos_id")),
            "qtd_recursos",
          ],
        ],
        include: [
          {
            ...
            required: true, // INNER JOIN → elimina null
            attributes: ["recursos"],
          },
        ],
        group: [
          "Evento.ano",
          "Evento.recursos_id",
          "ass_evento_recursos.id",
          "ass_evento_recursos.recursos",
        ],
      });
